...
AudioConnection          patchCord4(mixer1, 0, amp1, 0);
AudioConnection          patchCord5(amp1, 0, dacs1, 0);
...
void setup() {
  ...
  AudioMemory(20);
  waveform0.begin(current_waveform);
  ...
  // joysticks
   ...
   //   usbMIDI.sendControlChange(xAxisController, xAxisPin, channel);
        ...
        if (nextWaveform == 1 && waveformNum[0] > 0  )
          {
          waveform0.frequency(Frequency[waveformNum[0]]* frequencyMod /1000) ;
          }
        else if (nextWaveform == 2 && waveformNum[1] > 0  )
          {
          waveform1.frequency(Frequency[waveformNum[1]]* frequencyMod /1000) ;
          }
        else if (nextWaveform == 3 && waveformNum[2] > 0  )
          {
          waveform2.frequency(Frequency[waveformNum[2]]* frequencyMod /1000) ;
          }
        else if (nextWaveform == 0 && waveformNum[3] > 0  )
          {
          waveform3.frequency(Frequency[waveformNum[3]]* frequencyMod /1000) ;
          }
 ...
 //    Serial.println(nextWaveform);
  ...
  //   AudioNoInterrupts();
     if (nextWaveform == 0)
        ...
        waveform0.amplitude(initalAmplitude);
        waveform0.frequency(Frequency[midiVal]);
        nextWaveform = 1;
        }
        ...
        nextWaveform = 2;
        waveform1.amplitude(initalAmplitude);
        waveform1.frequency(Frequency[midiVal]);
        ...
        nextWaveform = 3;
        waveform2.amplitude(initalAmplitude);
        waveform2.frequency(Frequency[midiVal]);
        }
      else if  (nextWaveform == 3)
        {
        waveformNum[3] = midiVal;
        waveform3.amplitude(initalAmplitude);
        waveform3.frequency(Frequency[midiVal]);
        nextWaveform = 0;
        } 
 }
